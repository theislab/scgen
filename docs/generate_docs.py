...
scgen_dir = pathlib.Path(__file__).resolve().parents[1]
print(scgen_dir)
...
def clean_module_name(name):
    ...
    assert name[:6] == 'scgen.', 'Invalid module name: %s' % name
    return name
...
def class_to_source_link(cls):
    ...
    link = ('https://github.com/M0hammadL/'
            'scgen/blob/master/' + path + '#L' + str(line))
    return '[[source]](' + link + ')'
...
def generate(sources_dir):
    ...
    """
    template_dir = os.path.join(str(scgen_dir), 'docs', 'templates')
    ...
    # shutil.copyfile(os.path.join(str(scgen_dir), 'CONTRIBUTING.md'),
    #                 os.path.join(str(sources_dir), 'contributing.md'))
    # copy_examples(os.path.join(str(scgen_dir), 'examples'),
    #               os.path.join(str(sources_dir), 'examples'))


if __name__ == '__main__':
    generate(os.path.join(str(scgen_dir), 'docs', 'sources'))
