...
class CVAE:
    ...
    def __init__(self, x_dimension, z_dimension=100, **kwargs):
        ...
        self.source_x = tensorflow.placeholder(tensorflow.float32, shape=[None, self.x_dim], name="source_x")
        self.source_y = tensorflow.placeholder(tensorflow.float32, shape=[None, 1], name="source_y")
        self.dest_x = tensorflow.placeholder(tensorflow.float32, shape=[None, self.x_dim], name="dest_x")
        self.dest_y = tensorflow.placeholder(tensorflow.float32, shape=[None, 1], name="dest_y")
    ...
    def _encoder(self, x, y):
        ...
        with tensorflow.variable_scope("encoder", reuse=tensorflow.AUTO_REUSE):
            xy = tensorflow.concat([x, y], axis=1)
    ...
    def _sample_z(self, mu, log_var):
        ...
        return mu + tensorflow.exp(log_var / 2) * eps
    ...
    def _create_network(self):
        ...
        #normal flow
        self.mu, self.log_var = self._encoder(self.x, self.y)
        self.z_mean = self._sample_z(self.mu,  self.log_var)
        self.x_hat, self.mmd_hl = self._mmd_decoder(self.z_mean, self.y)

        #source flow
        self.mu_s, self.log_var_s = self._encoder(self.source_x, self.source_y)
        self.source_mean = self._sample_z(self.mu_s,  self.log_var_s)
        self.x_hat_s, self.mmd_s = self._mmd_decoder(self.source_mean, self.source_y)

        #destination flow
        self.mu_d, self.log_var_d = self._encoder(self.dest_x, self.dest_y)
        self.dest_mean = self._sample_z(self.mu_d, self.log_var_d)
        self.x_hat_d, self.mmd_d = self._mmd_decoder(self.dest_mean, self.dest_y)
    ...
    def _loss_function(self):
        ...
        self.mmd_loss = self.compute_mmd(self.mmd_s, self.mmd_d)
        self.vae_loss = tensorflow.reduce_mean(self.recon_loss + self.alpha * self.kl_loss + self.beta* self.mmd_loss)
    ...
    def train(self, train_data, use_validation=False, valid_data=None, n_epochs=25, batch_size=32, early_stop_limit=20,
              threshold=0.0025, initial_run=True, shuffle=True):#TODO: Write minibatches for each source and destination
        """
        ...
        min_sample = min([train_source.shape[0], train_dest[0]])
        ...
        for it in range(n_epochs):
            ...
            for lower in range(0, min_sample, batch_size):
                upper = min(lower + batch_size, min_sample)
                if sparse.issparse(train_data.X):
                    x_s = train_source[lower:upper, :].X.A
                    x_d = train_dest[lower:upper, :].X.A
                else:
                    x_s = train_source[lower:upper, :].X.A
                    x_d = train_dest[lower:upper, :].X.A
                y_mb = np.ones(x_d.shape[0], 1)
                _, current_loss_train = self.sess.run([self.solver, self.mm],
                                                      feed_dict={self.x: x_mb, self.y: y_mb,
                                                                 self.time_step: current_step,
                                                                 self.size: len(x_mb), self.is_training: True})
                train_loss += current_loss_train
