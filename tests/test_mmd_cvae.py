...
def test_train_whole_data_one_celltype_out(z_dim=50, alpha=0.001, beta=100, kernel="multi-scale-rbf", n_epochs=1000,
                                           batch_size=1024, cell_type_key="cell_type", condition_key="condition"):
    ...
    for cell_type in train.obs[cell_type_key].unique().tolist():
        ...
        net_train_data = train[~((train.obs[cell_type_key] == cell_type) & (train.obs[condition_key] == "stimulated"))]
        ...
        unperturbed_data = train[((train.obs[cell_type_key] == cell_type) & (train.obs[condition_key] == "control"))]
        ...
        sc.pl.umap(all_adata, color=condition_key, save="pred")
...
def reconstruct_whole_data(cell_type_key="cell_type", condition_key="condition"):
    train = sc.read("../data/ch10_train_7000.h5ad", backup_url="https://goo.gl/33HtVh")
    ...
    for cell_type in train.obs[cell_type_key].unique().tolist():
        ...
        cell_type_data = train[train.obs[cell_type_key] == cell_type]
        cell_type_ctrl_data = train[(train.obs[cell_type_key] == cell_type_data) & (train.obs[condition_key] == "control")]
        unperturbed_data = train[((train.obs[cell_type_key] == cell_type) & (train.obs[condition_key] == "control"))]
        true_labels = np.zeros((len(unperturbed_data), 1))
        fake_labels = np.ones((len(unperturbed_data), 1))
        pred = network.predict(data=unperturbed_data, encoder_labels=true_labels, decoder_labels=fake_labels)
        ...
        pred_adata = anndata.AnnData(pred, obs={condition_key: [f"{cell_type}_pred_stim"] * len(pred)},
                                     var={"var_names": cell_type_data.var_names})
        ctrl_adata = anndata.AnnData(ctrl_reconstructed, obs={condition_key: [f"{cell_type}_pred_stim"] * len(ctrl_reconstructed)},
                                     var={"var_names": cell_type_data.var_names})
