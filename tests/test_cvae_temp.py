...
def test_train_cross_study(
    ...
    pred_adata = anndata.AnnData(pred, obs={condition_key: ["pred"] * len(pred),
                                            "cell_type": control_data.obs["cell_type"].tolist()},
...
def test_train_whole_data_one_celltype_out(data_name="pbmc",
    ...
    for cell_type in train.obs[cell_type_key].unique().tolist():
        os.makedirs(f"./cvae_results/{data_name}/{cell_type}/", exist_ok=True)
        ...
        network = scgen.CVAE(x_dimension=net_train_data.X.shape[1], z_dimension=z_dim, alpha=alpha,
                             model_path=f"./")
